...
export async function convertFont(buffer) {
  try {
    ...
    // Try to convert to WOFF with proper WOFF format structure
    let compressedWoff;
    try {
      ...
      // Use font.toArrayBuffer for WOFF format instead of custom compression
      const woffBuffer = font.toArrayBuffer({ type: 'woff' });
      compressedWoff = Buffer.from(woffBuffer);
    } catch (woffError) {
      console.warn("WOFF conversion failed:", woffError);
      // Fallback: if WOFF2 is available, use it
      if (woff2Buffer) {
        console.log("Using WOFF2 as fallback");
        compressedWoff = Buffer.from(woff2Buffer);
      } else {
        // Last resort - try original TTF/OTF buffer
        console.log("Using original font as fallback");
        compressedWoff = Buffer.from(buffer);
      }
    }
